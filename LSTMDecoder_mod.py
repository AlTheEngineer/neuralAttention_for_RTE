...
from lasagne.layers import Gate, LSTMLayer
from lasagne.layers import InputLayer, DenseLayer, MergeLayer, Layer
...
class AttentionLayer(LSTMLayer):
    r"""
    lasagne.layers.recurrent.AttentionLayer(incoming, num_units,
    ...
    def __init__(self, incoming, num_units,
                 ...
                 backwards=False,
                 learn_init=False,
                 peepholes=False, # changed this to False, because I don't need it
                 gradient_steps=-1,
                 grad_clipping=0,
                 unroll_scan=False,
                 precompute_input=True,
                 mask_input=None,
                 only_return_final=True, # changed this to True, because I do need it
                 encoder_mask_input=None, # added for conditional encoding
                 attention = True, # added for bi-directional recurrent attention
                 wordbyword = True, # added for word-by-word attention
                 **kwargs):
       ...
       # If the provided nonlinearity is None, make it linear
        ...
        self.r_init = None
        self.r_init = self.add_param(init.Constant(0.), (1, num_units), name="r_init", trainable=False, regularizable=False)

        if self.attention:
            self.W_y = self.add_param(init.Normal(0.5), (num_units, num_units), 'W_y')
            self.W_h = self.add_param(init.Normal(0.5), (num_units, num_units), 'W_h')
            self.w = self.add_param(init.Normal(0.5), (num_units, 1), 'w')
            self.W_p = self.add_param(init.Normal(0.5), (num_units, num_units), 'W_p')
            self.W_x = self.add_param(init.Normal(0.5), (num_units, num_units), 'W_x')

            if self.wordbyword:
                self.W_r = self.add_param(init.Normal(0.5), (num_units, num_units), 'W_r')
                self.W_t = self.add_param(init.Normal(0.5), (num_units, num_units), 'W_t')
    ...
    def get_output_for(self, inputs, **kwargs):
        ...
        if self.attention:
            encoder_hs = cell_init[0] * encoder_mask.dimshuffle(0, 1, 'x')
        ...
        def step_masked(input_n, mask_n, cell_previous, hid_previous, *args):
            cell, hid, r = step(input_n, cell_previous, hid_previous, r_previous, *args)

            # Skip over any input with mask 0 by copying the previous
            # hidden state; proceed normally for any input with mask 1.
            cell = T.switch(mask_n, cell, cell_previous)
            hid = T.switch(mask_n, hid, hid_previous)
            r = T.switch(mask_n, r, r_previous)
        ...
        if self.unroll_scan:
            ...
            if self.attention and self.wordbyword:
                non_seqs += [self.W_y, self.W_h, self.W_r, self.w, self.W_t, encoder_hs]
            ...
            out = h_N
            r_N = r_out[-1]
            out = T.dot(r_N, self.W_p) + T.dot(h_N, self.W_x)
            out = nonlinearities.tanh(out)
        return out
